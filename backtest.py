# ─────────────────────────────────────────────────────────────
#  HRD Model — Historical Backtest (2022-2025)
#
#  Runs the full model pipeline for each year's actual field,
#  using that season's Statcast data and the correct host venue.
...
# Kill any Statcast fetch that hangs (e.g. after a network change)
...
from formatter import print_markets
from config import N_SIMULATIONS


# ── Historical fields ─────────────────────────────────────────
# park keys: hr_factor_R, hr_factor_L, dist_bonus_ft
# Notes on bat speed: Statcast bat speed tracking began in 2023.
#   For 2022 all players receive field-mean bat speed (effectively
#   removes that attribute as a discriminator for that year).

YEARS = {
    2022: {
        "venue":        "Nationals Park",
        "park":         {"hr_factor_R": 1.00, "hr_factor_L": 1.02, "dist_bonus_ft": 0.0},
        "season_start": "2022-04-07",
        "season_end":   "2022-10-05",
        "winner":       "Juan Soto",
        "players": [
...
def _set_park(park: dict) -> None:
    """Patch simulator module-level park factor globals for this year's venue."""
    sim_module._PARK_HR_FACTOR_R = park["hr_factor_R"]
    sim_module._PARK_HR_FACTOR_L = park["hr_factor_L"]
    sim_module._PARK_DIST_BONUS  = park["dist_bonus_ft"]


def _fmt_american(prob: float) -> str:
    if prob <= 0 or prob >= 1:
        return "N/A"
    if prob >= 0.5:
...
def run_year(year: int, cfg: dict) -> None:
    print(f"\n{'=' * 62}")
    print(f"  {year} HRD — {cfg['venue']}")
    print(f"  Park factors: R={cfg['park']['hr_factor_R']:.2f}  "
          f"L={cfg['park']['hr_factor_L']:.2f}  "
          f"dist bonus={cfg['park']['dist_bonus_ft']:+.1f}ft")
    ...
    # 2. Build player models
    players = build_player_models(players)

    # 3. Apply this year's park factors
    _set_park(cfg["park"])

    # 4. Simulate
    print(f"  Running {N_SIMULATIONS:,} simulations...", flush=True)
    t0 = time.time()
    sims = run_simulations(players, n=N_SIMULATIONS)
    print(f"  Simulations done in {time.time()-t0:.1f}s. Pricing markets...", flush=True)

    # 5. Price
    t1 = time.time()
    markets = price_all_markets(players, sims)
    print(f"  Pricing done in {time.time()-t1:.1f}s.", flush=True)

    # 6. Print all markets
    print_markets(markets)

    # Show where actual winner ranked
    actual = cfg["winner"]
    winner_market = markets.get("Winner", {})
    ranked = sorted(winner_market.items(), key=lambda x: x[1]["prob"], reverse=True)
    rank = next((i + 1 for i, (n, _) in enumerate(ranked) if n == actual), None)
    print(f"\n  *** Actual winner: {actual} (model rank: #{rank}) ***", flush=True)


def main():
    print("\nHRD Historical Backtest — 2022 to 2025")
    print("Note: 2022 bat speed data unavailable in Statcast (introduced 2023).\n")
